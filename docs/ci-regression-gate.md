# Patcharp CI, regression, security, and release gates

Patcharp owns six workflow entrypoints. They use public GitHub-hosted runners,
do not require AI/provider secrets, and keep required check names under company
control.

| Workflow | Purpose | Stable aggregate |
| --- | --- | --- |
| `patcharp-pr.yml` | changed-file routing, Rust/frontend checks, DB/API guards, fork delta | `patcharp_pr_gate` |
| `patcharp-contract.yml` | outer company gate: static contracts, security, and conditional full regression | `patcharp_contract_tests` |
| `patcharp-regression.yml` | reusable full SQLite, PostgreSQL, and product/API regression | `patcharp_regression_tests` |
| `patcharp-security.yml` | reusable Rust, JavaScript, license, and workflow supply-chain checks | `patcharp_security_gate` |
| `patcharp-release.yml` | immutable multi-architecture Docker Hub release | internal release gates |
| `upstream-drift.yml` | read-only upstream visibility | advisory only |

Only `patcharp_pr_gate` and `patcharp_contract_tests` are proposed as permanent
...
- `upstream-drift.yml`: Patcharp governance automation; read-only and never
  auto-merges upstream.

### Replace

- `build-pr-image.yml`, `build-fork-pr-image.yml`: replaced by the tag-only
  Patcharp release path; untrusted pull requests never publish images.
- `release.yml`: replaced by `patcharp-release.yml` and Docker Hub namespace
  `patcharp/openobserve`.

### Reuse capability

- `api-testing.yml`: reuse `tests/api-testing` and its client/fixtures.
- `audit-checker.yml`: reuse `cargo audit` in `patcharp-security.yml`.
- `cargo-deny.yml`: reuse cargo-deny and license policy.
- `db-schema-version-check.yml`: reuse
  `.github/scripts/check-db-schema-version.sh` in `patcharp-pr.yml`.
...
- `pure-api-spec-guard.yml`: reuse its classifier command in `patcharp-pr.yml`.
- `sbom-update.yml`: reuse SBOM capability in `patcharp-release.yml` without its
...
- Dependency automation not owned by Patcharp: `npm-update.yml`.
...
```bash
bash scripts/patcharp-contract.sh
```

Run one contract by basename while developing it:

```bash
bash scripts/patcharp-contract.sh fork-release-metadata
```
...
bash scripts/patcharp-product-smoke.sh
```
...
Pre-create the `patcharp/openobserve` Docker Hub repository before the first
...
tags matching `v*-patcharp.*`, require a maintainer reviewer, and store only:
...
cover every full Patcharp version/RC tag. The workflow also authenticates and
...
`patcharp-release.yml` validates an annotated tag, verifies it is on `main`,
checks the exact tagged SHA for `patcharp_pr_gate` and the outer
`patcharp_contract_tests`, generates provenance, rejects an existing Docker
...
Before pushing a release tag, wait until `patcharp_pr_gate` and
`patcharp_contract_tests` are green on the intended `main` SHA. The release
...
against the previous Patcharp release schema cannot be implemented until a
first supported Patcharp release exists. Historical-schema upgrade fixtures and
