...
| Build/version metadata | Existing `src/config/build.rs` provides upstream tag, commit, and build date; status/config responses are in `src/api/management/src/request/status/mod.rs`. PCPLAB metadata is generated independently and merged into the authenticated config/About response. | Immutable `ForkBuildMetadata` containing distribution, release channel, company release, fork SHA, upstream version/SHA/base type/security patches, build timestamp, license, and commit-pinned source location. Claimed releases validate fail-closed. | `.fork/upstream.env`, `scripts/fork-release-metadata.sh`, `src/pcplab/src/build_metadata/`, `web/src/pcplab/components/` | One workspace/API dependency, one field on the authenticated status response, and one About-page component hook. General config and anonymous bootstrap remain unaware of release policy. |
...
- A separate company migration/version path and contract test suite add ongoing
  ownership.
- GitHub branch/tag rulesets and real release-candidate evidence remain
  repository-administration/operations dependencies. The company regression
  gate, release artifact wiring, and in-product AGPL/source display are now
  executable contracts rather than documentation-only intent.
